...
async def benchmark(
    ...
    benchmark_request_sent_duration = last_request_sent_time - benchmark_start_time
...
def print_metrics(metrics: List[str], duration: float, namespace: str, job: str):
  ...
  server_metrics = {}
  print(f"job {job} namespace {namespace} duration {duration}")
...
async def print_and_save_result(args: argparse.Namespace, benchmark_duration, benchmark_request_sent_duration, total_requests, model, request_latencies, ttfts, itls, tpots, errors, scrape_metrics=False):
  ...
  print(f"Total request sent duration: {benchmark_request_sent_duration:.2f} s")
  ...
  benchmark_result['benchmark_request_sent_duration'] = benchmark_request_sent_duration
